...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Mobile overlay */}
      <div className={`lg:hidden fixed inset-0 bg-black/50 z-40 ${collapsed ? 'hidden' : ''}`} onClick={() => setCollapsed(true)} />

      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 flex flex-col bg-navy-900/95 backdrop-blur-sm border-r border-navy-700/50 transition-all duration-300 ${
          collapsed ? 'w-16' : 'w-60'
        }`}
      >
        {/* Logo */}
        <div className="flex items-center gap-3 px-4 h-16 border-b border-navy-700/50">
          ...
          <button
            ...
            className="ml-auto p-1.5 rounded-md hover:bg-navy-700/50 text-navy-400 hover:text-white transition-colors hidden lg:flex"
          >
        ...
        <nav className="flex-1 py-4 px-2 space-y-1">
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`w-5 h-5 flex-shrink-0 ${isActive ? 'text-accent-blue' : 'text-navy-400 group-hover:text-white'}`} />
        ...
        <div className="p-2 border-t border-navy-700/50">
          <button
            ...
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-navy-400 hover:text-red-400 hover:bg-red-500/10 transition-colors w-full"
